...
@SupportedAnnotationTypes("com.yumi.processor.BuildProperty")
public class BuildProcessor extends AbstractProcessor {
    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        System.out.println("BuildProcessor.process");
        ...
        for (TypeElement annotation : annotations) {
            ...
            setters.forEach(
                    setter -> {
                        ...
                        List<Element> elements = groupMap.computeIfAbsent(className, key -> new ArrayList<>());
                        elements.add(setter);
                    }
            );
            ...
            groupMap.forEach(
                    (key, value) -> {
                        Map<String, String> setterMap = value.stream().collect(
                          Collectors.toMap(
                        ...
                        try {
                            writeBuilderFile(key, setterMap);
                        } catch (IOException e) {
    ...
    private void writeBuilderFile(String className, Map<String, String> setterMap) throws IOException {
        ...
        try (PrintWriter out = new PrintWriter(builderFile.openWriter())) {
            ...
            setterMap.forEach((methodName, argumentType) -> {
                ...
                int set = methodName.indexOf("set") + 3;
                String buildMethodName = methodName.substring(set);
                char[] charArray = buildMethodName.toCharArray();
                charArray[0] = Character.toLowerCase(charArray[0]);
                buildMethodName = new String(charArray);

                out.print("    public ");
                out.print(builderSimpleClassName);
                out.print(" ");
                out.print(buildMethodName);
